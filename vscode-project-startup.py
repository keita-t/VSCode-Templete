...
# GitHubリポジトリ設定
DEFAULT_GITHUB_USER = "keita-t"
DEFAULT_REPO_NAME = "VSCode-Templete"
DEFAULT_BRANCH = "main"
...
def get_folder_mapping(template_name: str) -> Dict[str, str]:
    """テンプレートのフォルダマッピングを取得"""
    mapping = DEFAULT_FOLDER_MAPPING.copy()
    if template_name in TEMPLATE_FOLDER_MAPPINGS:
        mapping.update(TEMPLATE_FOLDER_MAPPINGS[template_name])
    return mapping


def get_file_mapping(template_name: str) -> Dict[str, str]:
    """テンプレートのファイルマッピングを取得"""
    mapping = DEFAULT_FILE_MAPPING.copy()
    if template_name in TEMPLATE_FILE_MAPPINGS:
        mapping.update(TEMPLATE_FILE_MAPPINGS[template_name])
    return mapping
...
class TemplateSource:
    ...
    def _download_from_github(self, template_path: str) -> Optional[bytes]:
        """GitHubからファイルをダウンロード"""
        url = f"https://raw.githubusercontent.com/{self.github_user}/{self.repo_name}/{self.branch}/{template_path}"
...
class TemplateSetup:
    ...
    def __init__(self, template_types: List[str], 
        ...
        self.merge_patterns = merge_patterns or DEFAULT_MERGE_PATTERNS
        self.project_dir = Path.cwd()
        
        # テンプレートソース
        token = load_github_token()
        self.source = TemplateSource(local_path=local_path, token=token)
    ...
    def _collect_files(self) -> bool:
        ...
        for template_name in self.template_types:
            folder_mapping = get_folder_mapping(template_name)
            file_mapping = get_file_mapping(template_name)
